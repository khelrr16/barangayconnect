...
<form method="POST" action="{{ route('committee.health.immunization.update', $infant) }}">
            ...
            <div class="table-responsive">
                <table class="table align-middle text-center">
                    ...
                    <tbody>
                        <tr>
                            <td>1 mo</td>
                            <td><input type="date" name="iron_1" class="form-control"></td>
                        </tr>
                        <tr>
                            <td>2 mo</td>
                            <td><input type="date" name="iron_2" class="form-control"></td>
                        </tr>
                        <tr>
                            <td>3 mo</td>
                            <td><input type="date" name="iron_3" class="form-control"></td>
                        </tr>
                    </tbody>
                </table>
            </div>
                    ...
                    <thead class="table-primary">
                        <tr>
                            ...
                            @foreach ($doses as $dose)
                                <th>
                                    <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#months13-{{ $dose }}-modal">
                                        {{ $dose }} Dose
                                    </button>
                                </th>
                            @endforeach
    ...
    <div class="text-end mt-3">
        <button type="submit" class="btn btn-success">Save 1-3 Months Tab</button>
    </div>
</form>

@foreach ($doses as $dose)
    <div class="modal fade" id="months13-{{ $dose }}-modal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                        ...
                        @if ($medicineId)
                            <div class="form-check mb-2">
                                <input
                                    ...
                                    type="checkbox"
                                    id="months13-modal-{{ $medicineId }}-{{ $dose }}"
                                    data-target-input="#months13-{{ $medicineId }}-{{ $dose }}"
                                >
                                <label class="form-check-label" for="months13-modal-{{ $medicineId }}-{{ $dose }}">
                                    {{ $medicineName }}
                                </label>
                            </div>
                        @endif
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-primary" onclick="applyDoseSelection('months13-{{ $dose }}-modal')" data-bs-dismiss="modal">Apply selection</button>
                </div>
            </div>
        </div>
    </div>
