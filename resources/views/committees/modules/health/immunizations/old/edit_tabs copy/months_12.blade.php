...
<form method="POST" action="{{ route('committee.health.immunization.update', $infant) }}">
                ...
                <table class="table table-striped align-middle text-center">
                    <thead class="table-primary">
                        <tr>
                            <th>Vaccine</th>
                            <th>
                                <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#months12-dose2-modal">
                                    2nd Dose
                                </button>
                            </th>
                        </tr>
                        ...
                        @foreach ($rows as $medicineName)
                            ...
                            @if ($medicineId)
                                <tr>
                                    ...
                                    <td>
                                        <input
                                            id="months12-{{ $medicineId }}-{{ $doseNumber }}"
                                            type="date"
                                            class="form-control"
    ...
    <div class="text-end mt-3">
        <button type="submit" class="btn btn-success">Save 12 Months Tab</button>
    </div>
</form>

<div class="modal fade" id="months12-dose2-modal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Select 2nd Dose Medicines</h5>
                    ...
                    @if ($medicineId)
                        <div class="form-check mb-2">
                            <input
                                ...
                                type="checkbox"
                                id="months12-modal-{{ $medicineId }}-{{ $doseNumber }}"
                                data-target-input="#months12-{{ $medicineId }}-{{ $doseNumber }}"
                            >
                            <label class="form-check-label" for="months12-modal-{{ $medicineId }}-{{ $doseNumber }}">
                                {{ $medicineName }}
                            </label>
                        </div>
                    @endif
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary" onclick="applyDoseSelection('months12-dose2-modal')" data-bs-dismiss="modal">Apply selection</button>
            </div>
        </div>
    </div>
</div>
